...
#define W_RAD 0.00917
#define W_DST 0.0408
#define PI 3.1415926

//crusing close loop parameter begin
...
#define D2OR_TOL 0.001
#define FX_F 0.01
#define DIST_P 300
#define DIST_P_LL 0.2

//crusing close loop parameter end
...
float d2org_diff[2];//{each PX DAT:{denoised data,{raw data shift}}}
long pxdat[8];//{each PX DAT:{denoised data,{raw data shift}}}
unsigned long crus_cyc = 0;
float wtd = 0;
float vR = 0, vL = 0;
LED* led[9];
Pen* pen;
Robot* robot;
unsigned short jnw = 0;
bool wallSide = R;
...
//void s_vel(float tar_L, float tar_R);
	...
	while (robot->step(timeStep) != -1) {
		...
		*/
		...
		if (posdat[0].Y > 0 && posdat[1].Y < 0) {
			...
			pen->setInkColor(0xff0000, 0.4);

		}
		trd = ((getThetaToOrg()) / 2 / PI + 0.6 + crus_cyc) * W_DST;
		d2org_diff[1] = d2org_diff[0];
		d2org_diff[0] = getDtoOrg() - trd;
		/*
		calculate open loop wheel condition
		*/
		...
		/*
		claculate correnction
		*/
		float fx_p = (d2org_diff[0] * P_CRUS);
		//limit correction
		//fx_p = (fx_p > (_VEL / W_RAD)) ? (_VEL / W_RAD) : fx_p;

		fx_i += (d2org_diff[0] * I_CRUS);
		float fx_d = (d2org_diff[0] - d2org_diff[1]) * D_CRUS;
		/*
		apply correction
		*/
		/*
		VL -= fx_p + fx_i - fx_d;
		VR += fx_p + fx_i - fx_d;// + (rad_err * P_CRUS);
		*/

		VR += abs(d2org_diff[0]) < D2OR_TOL ? 0 : d2org_diff[0] > 0 ? FX_F : -FX_F;
		if (abs(d2org_diff[0]) < D2OR_TOL) {
			led[8]->set(d2org_diff[0] > 0 ? 0xFFFF00 : 0xFF00FF);
		}
		else {
			led[8]->set(0xFF0000);
		}
		...
		if (detectWall(px, pxdat) && jnw==0) {
			...
			while (robot->step(timeStep) != -1 && ((dst_diff[0] > _MARG || ((dst_diff[1] - dst_diff[0]) < 0)) || jnw != 1)) {
				...
				if (jnw == _SHMITT_CYC) {
					...
					//s_vel(wallSide ? 1 : -1, wallSide ? -1 : 1);
					//cout << "pxdat[pxd_lkup]:\t" << pxdat[pxd_lkup] << endl;
					//if ((abs(pxdat[pxd_lkup + 1] - pxdat[pxd_lkup - 1]) < PRLL_MARG) || (pxdat[pxd_lkup] < _TH))jnw -= 1;
					if ((pxdat[pxd_lkup] > _TH))jnw -= 1;
					else continue;
				}
				else if (jnw == _SHMITT_CYC - 1) {

					if (dst_diff[0] > _MARG)jnw -= 1;
				...
				/*motor Drive*/
				float dist_p_fact = (dst_diff[0] - _MARG) * DIST_P;
				dist_p_fact = ((dist_p_fact > 1) || (dst_diff[0] > dst_diff[1])) ? 1 : dist_p_fact;
				dist_p_fact = (dist_p_fact < DIST_P_LL) ? DIST_P_LL : dist_p_fact;
				mot[R]->setVelocity(VR_ * dist_p_fact);
				mot[L]->setVelocity(VL_ * dist_p_fact);
				//s_vel(VL_, VR_);
			...
			while (robot->step(timeStep) != -1 && orediff > ORE_TOL) {
				mot[R]->setVelocity((ret_dir ? 1 : -1) * (orediff + 0.2));
				mot[L]->setVelocity((ret_dir? -1: 1) * (orediff + 0.2));
				//s_vel((ret_dir ? -1 : 1) * (orediff + 0.2), (ret_dir ? 1 : -1) * (orediff + 0.2));
				updatePose();
		...
		*/
		...
		//if (posdat[0].Y > 0 && posdat[1].Y < 0) {
			...
			cout << "rad_err: " << d2org_diff[0] << "\ttrd: " << trd << "\tVL: " << VL << "\tVR: " << VR << "\tcrus_cyc: " << crus_cyc << endl;
			/*for (char i = 0; i < 8; i++) {
				cout << pxdat[i] << '\t';
			}*/
			//print(posdat[0]);
			//printf("%f\n",diff_time);
			//cout  << getThetaToOrg ()<< endl;
		//}
	};
...
void s_vel(float tar_L, float tar_R) {
	static float V_R = 0, V_L = 0;
	V_R += ((V_R > tar_R) ? -1 : 1) * CORR_VEL;
	V_L += ((V_L > tar_L) ? -1 : 1) * CORR_VEL;
	mot[R]->setVelocity(V_R);
	mot[L]->setVelocity(V_L);
